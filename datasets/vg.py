...
if __name__ == "__main__":
    ...
    for item in tqdm(dataset):
        ...
        b0 = boxes[edges[:, 0]]
        b1 = boxes[edges[:, 1]]
        iou = box_iou(b0, b1)[0].diag()
        tmp = torch.where(iou > 0)[0]
        cnt[0] += iou.shape[0]
        cnt[1] += iou.shape[0] - tmp.shape[0]
    ...
    #freq_bias = FrequencyBias(stats)

    cnt = []
    zz = json.load(open("data/visual_genome/vg150_poly_gpt4sgg.json"))
    ids = set([e['image_id'] for e in zz])

    for (imgs, item) in tqdm(dataset):
        if str(item['image_id']) not in ids:
            continue 
        #if len(item['relations']) == 0 or len(item['boxes']) < 2:
        #    import pdb; pdb.set_trace()
        ...
        nouns = set(nouns)
        rels = list(set(rels))
        cnt.append(len(item['relations']))
        #if len(rels) > 3 and 'skateboard' in nouns:
        #    import pdb; pdb.set_trace()
    ...
    dst = []
    for item in tqdm(dataset):
        edges = item[1]['edges'].tolist()
        
        ee = []
        for edge, rel in zip(edges, item[1]['relations']):
            s = rel[0] + '.' + str(edge[0] + 1)
            o = rel[1] + '.' + str(edge[1] + 1)
            r = rel[2]
            tmp = {'source': s, "target": o, "relation": r}
            ee.append(tmp)
        dst.append({'image_id': item[1]['image_id'], 'gt': ee})
    ...
    for item in tqdm(dataset.images):
        if item['coco_id'] is not None:
            coco_ids_in_vg.append(item['coco_id'])
    ...
    rels_num  = []
    for item in tqdm(dataset):
        img_size = item[0].size
        boxes = item[1]['boxes']
        names = item[1]['gt_names']
        im_id = item[1]['image_id']

        edges = item[1]['edges'].tolist()
        tmp = []
        for rel in edges:
            tmp.append({'source':names[rel[0]] + ".%s"%(rel[0]+1), 
                        'target':names[rel[1]] + ".%s"%(rel[1]+1),
                        'relation': VG150_PREDICATES[rel[2]]})
        edges = tmp 
        tmp = {'image_id': im_id, 'boxes': boxes.tolist(), 'names': names,
                'width': item[0].width, 'height': item[0].height,
                'edges': edges}
        dst.append(tmp)
        rels_num.append(len(item[1]['edges']))


    import pdb; pdb.set_trace()
    with open("vg150_val.json", 'w') as fout:
        json.dump(dst, fout)
    import pdb; pdb.set_trace()
    freq = {}
    rels = {}
    zz = 0
    for (imgs, item) in tqdm(dataset):
        for rel in item['edges'].tolist():
            zz += 1
            name = dataset.ind_to_predicates[rel[2]]
            if name not in rels:
                rels[name] = 0
            rels[name] += 1
