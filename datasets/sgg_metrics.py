# modified from https://github.com/KaihuaTang/Scene-Graph-Benchmark.pytorch/blob/master/maskrcnn_benchmark/data/datasets/evaluation/vg/sgg_eval.py
...
from util.bounding_box import intersect_2d, argsort_desc, bbox_overlaps

from abc import ABC, abstractmethod

from util.misc import get_rank, get_world_size, all_gather
...
class SceneGraphEvaluation(ABC):
    ...
    def synchronize(self, name):
        ...
        # serialize and gather the data
        data = self.result_dict[name]
...
"""
class OvdSGZeroShotRecall(SceneGraphEvaluation):
    def __init__(self, result_dict):
        super().__init__(result_dict)
        self.novel_count = 0
        self.all_count = 0
        self.key = '_ovd_zeroshot_recall'

    def register_container(self, mode):
        self.result_dict[mode + self.key] = {20: [], 50: [], 100: []} 
    ...
    def generate_print_string(self, mode):
        ...
        #
        result_str = 'SGG eval : '
        for k, v in self.result_dict[name].items():
            result_str += '   zR @ %d: %.4f; ' % (k, np.mean(v))
        result_str += ' for mode=%s, type=Ovd Zero Shot Recall.' % mode
        result_str += '\n'
        return result_str
    ...
    def prepare_zeroshot(self, global_container, local_container):
        ...
        unseen_obj_cats = [9, 10, 11, 20, 22, 23, 26, 27, 28, 37, 38, 40, 41, 45, 52, 53, 54, 57, 58, 60, 61, 64, 66, 74, 78, 87, 91, 95, 97, 99, 111, 112, 113, 115, 121, 124, 126, 127, 134, 135, 136, 144, 145, 149, 150]
        for i, (s, o, p) in enumerate(gt_triplets):
            # if s not in unseen_obj_cats and o not in unseen_obj_cats:
            if s in unseen_obj_cats and o in unseen_obj_cats:
    ...
    def calculate_recall(self, global_container, local_container, mode):
        ...
        for k in self.result_dict[mode + self.key]:
            # Zero Shot Recall
            match = reduce(np.union1d, pred_to_gt[:k])
            if len(self.zeroshot_idx) > 0:
                if not isinstance(match, (list, tuple)):
                ...
                zeroshot_match = len(self.zeroshot_idx) + len(match_list) - len(set(self.zeroshot_idx + match_list))
                zero_rec_i = float(zeroshot_match) / float(len(self.zeroshot_idx))
                self.result_dict[mode + self.key][k].append(zero_rec_i)


class OvrSGZeroShotRecall(OvdSGZeroShotRecall):
    def __init__(self, result_dict, **kwargs):
        super().__init__(result_dict)
        self.novel_count = 0
        self.all_count = 0
        self.unseen_rels = kwargs.get("unseen_rels", 
                [9, 36, 38, 45, 22, 30, 34, 37, 16, 46, 39, 7, 14, 31, 49])
        self.key = '_ovr_zeroshot_recall'
    ...
    def generate_print_string(self, mode):
        ...
        for k, v in self.result_dict[name].items():
            result_str += '   zR @ %d: %.4f; ' % (k, np.mean(v))
        result_str += ' for mode=%s, type=Ovr Zero Shot Recall.' % mode
        result_str += '\n'
        return result_str
...
class SGPairAccuracy(SceneGraphEvaluation):
    ...
    def generate_print_string(self, mode):
        # sync. 
        self.synchronize(mode + '_accuracy_hit')
        self.synchronize(mode + '_accuracy_count')
        # 
...
class SGMeanRecall(SceneGraphEvaluation):
    def __init__(self, result_dict, num_rel, ind_to_predicates, print_detail=False):
        ...
        self.rel_name_list = copy.deepcopy(ind_to_predicates[1:]) # remove __background__
    ...
    def calculate_mean_recall(self, mode):
        # sync. 
        name = mode + '_mean_recall_collect'
        world_size = get_world_size()
        data_list = all_gather(self.result_dict[name])
        for i in range(world_size):
            if i != get_rank():
                data = data_list[i]
                for k, v in data.items():
                    for j in range(len(v)):
                        self.result_dict[name][k][j] += v[j]
...
class SGNGMeanRecall(SceneGraphEvaluation):
    ...
    def calculate_mean_recall(self, mode):
        # sync. 
        name = mode + '_ng_mean_recall_collect'
        world_size = get_world_size()
        data_list = all_gather(self.result_dict[name])
        for i in range(world_size):
            if i != get_rank():
