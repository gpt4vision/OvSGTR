...
def get_param_dict(args, model_without_ddp: nn.Module):
    ...
    if param_dict_type == 'default':
        ...
        def is_rln_param(name):
            return 'rln' in name or 'relation_cross_attn' in name or 'dropout12' in name or 'norm12' in name or 'relation' in name

        param_dicts = [
            {"params": [p for n, p in model_without_ddp.named_parameters() if "backbone" not in n and 
                                                               (not is_rln_param(n)) and p.requires_grad]},
            {
                "params": [p for n, p in model_without_ddp.named_parameters() if "backbone" in n and p.requires_grad],
                "lr": args.lr_backbone,
            },
            {
                "params": [p for n, p in model_without_ddp.named_parameters() if is_rln_param(n) and p.requires_grad],
                "lr": args.lr * getattr(args, "lr_rln_mult", 1.0)
            }
        ]
        return param_dicts
